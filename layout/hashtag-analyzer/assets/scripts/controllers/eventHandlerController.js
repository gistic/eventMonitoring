    ...
    function ($rootScope, $scope, $http, $location, $window, $anchorScroll, $state, RequestData, CreateEventSource, $timeout, SweetAlert, SweetAlertFactory, ISO3166, Lightbox, $modal, $sce, $cookies, $cookieStore, languageCode, User, filterHashtags) {
        ...
        $scope.downloadPdfReport = function () {
            ...
            var doc = new jsPDF(/*'landscape'*/), 
                      ...
                      imgSize = {},
                      imgeType = 'JPEG'
                      panelIds = ['tweets-per-country-panel' ,'tweets-overtime-panel', 'languages-panel', 'tweets-location-panel' , 'tweets-sources-panel', 'top-people-panel', 'hashtags-cloud-panel'],
                      date = new Date(),
                      ...
                      reportEmails = 'yourEmailIds@mail.com',
                      reportMonitor = 'setMonitor',
                      reportCreator = 'setCreator',
                      reportFrequency = 'setFrequency',
            ...
            doc.setFontSize(18);
            doc.text(marginLeft + logoWidth + padding, marginTop, "REPORT FOR '{0}' KEYWORDS.".replace('{0}', $('#eventHashtag').val()));

            doc.setFontSize(10);
            doc.text(marginLeft + logoWidth + padding, (marginTop += lineHeight), 'Report created for monitor: ' + reportMonitor + ' at: ' + (date.getHours() + ":"  + date.getMinutes()) + ' by: ' + reportCreator + ' on ' + reportFrequency + ' basis.');
            ...
            // TODO: Combine both functions in a single unit of code.

            function addImage(element, imgSize, marginLeft, marginTop, isLast) {
                    domtoimage
                        .toPng(element)
                        .then(function (dataUrl) {
                            ...
                            canvas.width = img.width < imgSize.width ? img.width : imgSize.width;
                            canvas.height = img.height < imgSize.height ? img.height : imgSize.height;

                            ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
                            images.push(canvas);                    
                            doc.addImage(canvas.toDataURL("image/jpeg"), imgeType, marginLeft, marginTop+2);

                            if (images.length > childCount) {
                                images = [];
                                marginTop = 20, 
                                doc.addPage();
                                addPanels();
                    ...
                    if (element != null) {  
                        domtoimage.toPng(element)
                            .then(function (dataUrl) {
                                ...
                                if (images.length == panelIds.length) {
                                    for (var i = 0; i < images.length; i++) {
                                        if (i == 0 || i == 4) {
                                            marginTop = 5;
                                        }

                                        doc.addImage(images[i].toDataURL("image/jpeg"), imgeType, i < 4 ? marginLeft : marginLeft + pageWidth*0.49, marginTop+2);
                                        marginTop += 70;  
                                    }
                                    doc.save(reportName);
                                }                       
